...
sealed trait ReferenceChecker {
  ...
  implicit class StringImprovements(s: String) {
    def dropCharAtIndex(n: Int): String = {
...
trait LetterRemainderLookup extends ModulusReferenceChecker {
  val remainderLookupTable: List[Char] =
    List('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'X', 'J', 'K', 'L', 'M', 'N', 'Y', 'P', 'Q', 'R', 'S', 'T', 'Z', 'V', 'W')
  val modulus = 23
}

trait UtrReferenceChecker extends ModulusReferenceChecker {
  val remainderLookupTable: List[Char] = List('2', '1', '9', '8', '7', '6', '5', '4', '3', '2', '1')
  val modulus = 11
  val checkCharIndex = 0

  val weights: List[Int] = List(6, 7, 8, 9, 10, 5, 4, 3, 2)
  val letterToNumber = Map.empty[Char, Int]
  val refRegex = """^\d{10}$"""

  override def referenceToValidate(reference: String): String = reference.tail
}

object SelfAssessmentReferenceChecker extends UtrReferenceChecker {
  override def prepareReference(reference: String): String =
...
object VatReferenceChecker extends DifferenceReferenceChecker {
  val weights: List[Int] = List(8, 7, 6, 5, 4, 3, 2)
  val letterToNumber = Map.empty[Char, Int]
  val refRegex = """^\d{9}$"""
}
...
object EpayeReferenceChecker extends LetterRemainderLookup {
  ...
  val weights: List[Int] = List(9, 10, 11, 12, 0, 8, 7, 6, 5, 4, 3, 2, 1)
  val letterToNumber: Map[Char, Int] = Map(
    'A' -> 33,
    'B' -> 34,
    'C' -> 35,
    'D' -> 36,
  ...
  override def extraChecks: List[String => Boolean] = super.extraChecks :+
...
object OtherTaxReferenceChecker extends LetterRemainderLookup {
  ...
  val weights: List[Int] = List(0, 0, 9, 10, 11, 12, 13, 8, 7, 6, 5, 4, 3, 2, 1)
  val letterToNumber: Map[Char, Int] = Map(
    'A' -> 33,
    'B' -> 34,
    'C' -> 35,
